...
class AIService:
    ...
    def _get_model(cls):
        ...
        if cls._model is None:
            ...
            if is_gpu:
                 ...
                 logger.info("GPU detected. Using Medium model (yolo11m-cls.pt) for better accuracy.")
                 # Download URL for YoloV11 Medium
                 url = "https://github.com/ultralytics/assets/releases/download/v8.3.0/yolo11m-cls.pt"
            else:
                 model_name = "yolo11n-cls.pt"
                 logger.info("No GPU detected. Using Nano model (yolo11n-cls.pt) for speed.")
    ...
    def save_feedback(content: bytes, product_id: int):
        ...
        with open(os.path.join(settings.DATASET_DIR, str(product_id), filename), "wb") as f:
            f.write(content)  

    @staticmethod
    def _prepare_training_dataset(source_dir: str, dest_dir: str, split_ratio: float = 0.8):
        """
        ...
        for class_name in os.listdir(source_dir):
            ...
            images = [f for f in os.listdir(class_path) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.webp'))]
            ...
            if not val_imgs:
                ...
                raise HTTPException(status_code=400, detail=f"Dataset too small for class '{class_name}'. Must have at least 2 images to perform a train/val split. Found {len(images)} images.")
    ...
    def retrain_model(cls):
        ...
        # Use /data/temp instead of MEDIA_ROOT to keep static files clean
        temp_dataset_dir = os.path.join("/data", "temp", "training_data_" + str(uuid.uuid4()))
        project_dir = os.path.join(settings.MODELS_DIR, "training_runs")
        run_name = "latest_run"
        
        try:
            has_data = cls._prepare_training_dataset(settings.DATASET_DIR, temp_dataset_dir)
            ...
            if os.path.exists(new_model_path):
                logger.info(f"Training completed. New model found at {new_model_path}. Updating...")
