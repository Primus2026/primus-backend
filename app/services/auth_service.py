...
class AuthService:
    ...
    def create_login_token(user: User) -> Token:
        if user.is_2fa_enabled:
            ...
            access_token = security.create_access_token(
                ...
                claims={"2fa_required": True},
            )
            return Token(
                access_token=access_token, token_type="bearer", is_2fa_required=True
            )
        ...
        return Token(
            access_token=access_token, token_type="bearer", is_2fa_required=False
        )
    ...
    async def setup_2fa(db: AsyncSession, user: User) -> dict[str, str]:
        ...
        uri = pyotp.totp.TOTP(secret).provisioning_uri(
            name=user.email, issuer_name="Primus 2026"
        )
